...
public class CommunalService {
     ...
     *
     * @param utilitiesPayload входные данные с идентификатором имущества, и кол-вом затраченных коммунальных услуг
    ...
    public ResponseEntity<UtilityBill> calculateUtilityBill(UtilitiesPayload utilitiesPayload) {
        ...
        Property property = this.propertyRepository.findPropertyByPropertyId(utilitiesPayload.getPropertyId());
        if (property == null) {
            logger.error("Property with ID = {} not found", utilitiesPayload.getPropertyId());
            return ResponseEntity.status(400).body(utilityBill);
        }

        UtilitiesPriceList utilitiesPriceList = this.utilitiesPriceListRepository
            .findUtilitiesPriceListByRegion(property.getRegion());
        if (utilitiesPriceList == null) { // сделать в скрипте общий прайс-лист и юзать его, если не находится региональный
        ...
        utilityBill.setColdWaterAmount(utilitiesPayload.getColdWater() * utilitiesPriceList.getHotWaterPrice());
        utilityBill.setHotWaterAmount(utilitiesPayload.getHotWater() * utilitiesPriceList.getHotWaterPrice());
        ...
        try {
            Long paymentRequestId = this.bankService.requestPayment(
                new PaymentPayload(
                    21L,
