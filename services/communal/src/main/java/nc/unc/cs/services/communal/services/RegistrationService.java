...
public class RegistrationService {
  ...
  addRegistration(final Registration registration) {
    ...
    try { // ? id вытаскивать из токена или получать с фронта ?
      ...
      response =
          ResponseEntity.status(HttpStatus.BAD_REQUEST).body(registration);
   ...
   * @return недвижимость
   */
  public Property getPropertyByAddress(final Property property)
  ...
  public ResponseEntity<Property> addCitizensProperty(final Property property) {
    ...
    try { // сохранения недвижимости после регистрации услуги
      ...
      logger.error("Failed to privatize property!"
                   ...
                   fe);
      response =
          ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(property);
    }
    return response;
  }
