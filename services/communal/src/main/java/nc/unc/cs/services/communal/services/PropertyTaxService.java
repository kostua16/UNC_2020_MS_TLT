...
public class PropertyTaxService {
  ...
  addPropertyTaxValue(final PropertyTaxValue propertyTaxValue) {
    ResponseEntity<PropertyTaxValue> response;
    if (propertyTaxValue.getCadastralValue() != null &&
      ...
      response =
          ResponseEntity.status(HttpStatus.BAD_REQUEST).body(propertyTaxValue);
    }
    return response;
  }
  ...
  changePropertyTaxStatus(final Long propertyTaxId) {
    ...
    try {
      if (this.bankIntegrationService.checkPaymentStatus(
