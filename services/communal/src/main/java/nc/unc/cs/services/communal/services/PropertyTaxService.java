...
public class PropertyTaxService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(PropertyTaxService.class);
  ...
  addPropertyTaxValue(final CreationPropertyTaxValue newPropertyTaxValue) {
    ...
    if (lastPropertyTaxValue == null) {
      ...
      lastPropertyTaxValue.setCadastralValue(
          propertyTaxValue.getCadastralValue());
      lastPropertyTaxValue.setPricePerSquareMeter(
          propertyTaxValue.getPricePerSquareMeter());
  ...
  getPropertyTaxValueById(final Long propertyTaxValueId) {
   ...
   */
  public Integer calculatePropertyTaxAmount(final Double apartmentSize,
                                            final Double pricePerSquareMeter,
                                            final Double cadastralValue) {
    return (int)(apartmentSize * pricePerSquareMeter / PERCENT_DIVISOR *
   ...
   * @throws NullPointerException если не удасться найти недвижимостьея или
   *     прейскурант
   */
  public ResponseEntity<PropertyTax>
  calculatePropertyTax(final Long propertyId) {
    ...
    try {
      ...
      final PropertyTax propertyTax =
          PropertyTax.builder()
              ...
              .date(new Date())
              .taxAmount(amount)
              .paymentRequestId(this.bankIntegrationService.bankRequest(
   ...
   * @return http-ответ, в теле которого находится изменённая налоговая
   *     квитанция
   ...
   */
  public ResponseEntity<PropertyTax>
  changePropertyTaxStatus(final Long propertyTaxId) {
    ResponseEntity<PropertyTax> response;
    final PropertyTax propertyTax = this.getPropertyTaxById(propertyTaxId);

    try {
      final Boolean isPaid = this.bankIntegrationService.checkPaymentStatus(
      ...
      if (Boolean.TRUE.equals(isPaid)) {
        ...
        logger.error("PropertyTax with ID = {} was not paid",
                     propertyTax.getPropertyTaxId());
        response = ResponseEntity.status(HttpStatus.PAYMENT_REQUIRED)
                       .body(propertyTax);
      }
    } catch (NullPointerException npe) {
      logger.error("PropertyTax with ID = {} not found", propertyTaxId, npe);
      response =
          ResponseEntity.status(HttpStatus.BAD_REQUEST).body(propertyTax);
    } catch (FeignException fe) {
  ...
  /**
   * Возвращает все налоговые квитанции на недвижимость для конкретного
   * гражданина.
   *
   ...
   * имущества.
   *
   * @param propertyId идентификатор недвижимости, на которую были расчитаны
   *     налоги
   * @return Список налоговых квитанций
   */
  public List<PropertyTax>
  getDebtPropertyTaxesByProperty(final Long propertyId) {
    return this.propertyTaxRepository.findPropertyTaxesByPropertyIdAndIsPaid(
        propertyId, false);
  }
  ...
  /**
   * Возвращает налоговую квитанции на недвижимость по указаноому
   * идентификатору.
   *
  ...
  public PropertyTax getPropertyTaxById(final Long propertyTaxId) {
    return this.propertyTaxRepository.findPropertyTaxByPropertyTaxId(
        propertyTaxId);
  }
}
