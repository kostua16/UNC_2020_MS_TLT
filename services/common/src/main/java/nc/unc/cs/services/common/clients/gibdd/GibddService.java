...
@FeignClient(name = "GIBDD", path = "gibdd",
             url = "${app.gibdd-url:localhost:8080}")
...
public interface GibddService {
  ...
  ResponseEntity<CarDto>
  addCar(@RequestBody @Validated @NotNull CarDto dto);

  @PostMapping(value = "cars", consumes = "application/json",
               produces = "application/json")
  ResponseEntity<CarDto>
  updateCar(@RequestBody @Validated CarDto dto);
}
