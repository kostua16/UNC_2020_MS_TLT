...
public class TaxController {
  ...
  public Boolean checkPaid(
      @Parameter(name = "taxId", description = "Tax ID", required = true, schema = @Schema(type = "long"))
      @PathVariable("taxId") final Long taxId) {
    return this.taxService.isPaid(taxId);
  }

  @PostMapping(value = "create", produces = "application/json", consumes = "application/json")
  @Operation(summary = "createTax", description = "Tax creation for a specific service", method = "POST")
  ...
  @ApiResponses(
      @ApiResponse(responseCode = "400", description = "Tax ID")
  )
