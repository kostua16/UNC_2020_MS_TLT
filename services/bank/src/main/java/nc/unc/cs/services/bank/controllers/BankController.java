...
public class BankController {
  ...
  @ApiOperation(
      httpMethod = "POST", value = "Registration of provided services",
      notes =
          "Registers taxes and creates an invoice for payment (PaymentRequest)",
      //        response = ResponseEntity.class,
      nickname = "requestPayment")
  @ApiResponses({
    @ApiResponse(code = 500, message = "PaymentRequest with ID = null",
                 response = PaymentRequest.class)
    ,
        @ApiResponse(code = 503, message = "PaymentRequest with ID = null",
                     response = PaymentRequest.class)
  })
  @ApiImplicitParam(
      name = "paymentPayload",
      value = "Data for registration of the service provided to the user",
      required = true, type = "PaymentPayload", dataType = "PaymentPayload",
      dataTypeClass = PaymentPayload.class, paramType = "body")
  public ResponseEntity<Long>
  requestPayment(@RequestBody final PaymentPayload paymentPayload) {
    return this.bankService.requestPayment(
        paymentPayload.getServiceId(), paymentPayload.getCitizenId(),
        paymentPayload.getAmount(), paymentPayload.getTaxAmount());
  }

  @ApiOperation(
      httpMethod = "PUT", value = "Payment of the invoice",
      notes = "Payment of the issued invoice and the tax attached to it",
      response = Transaction.class, nickname = "pay")
  @ApiResponses({
    @ApiResponse(code = 400, message = "Uncreated transaction",
                 response = Transaction.class)
    ,
        @ApiResponse(code = 500, message = "Uncreated transaction",
                     response = Transaction.class),
        @ApiResponse(code = 503, message = "Uncreated transaction",
                     response = Transaction.class)
  })
  @PutMapping(value = "payment/{paymentId}", produces = "application/json")
  public ResponseEntity<Transaction>
  pay(@ApiParam(name = "paymentId", type = "long", value = "PaymentPayload ID",
                required = true) @PathVariable("paymentId")
  ...
  public Boolean
  checkPaymentStatus(@ApiParam(name = "paymentId", type = "long",
                               value = "PaymentPayload ID", required = true)
  ...
  @ApiOperation(httpMethod = "GET",
                ...
                response = PaymentRequest.class, responseContainer = "List")
  @GetMapping("check/{citizenId}")
  public List<PaymentRequest>
  getDebtPaymentRequests(@ApiParam(name = "citizenId", type = "long",
