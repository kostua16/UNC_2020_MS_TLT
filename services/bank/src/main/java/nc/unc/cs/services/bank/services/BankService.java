...
public class BankService {
  ...
  /**
   * @param paymentId The ID of the payment by which the payment is searched in
   *     the database.
   */
  public PaymentRequest findPaymentRequestById(Long paymentId) {
    return this.paymentRequestRepository.findById(paymentId).orElseThrow(
   ...
   * @return Payment ID;
   */
  public ResponseEntity<Long> requestPayment(final Long serviceId,
    ...
    try {
      Long taxId = this.taxService.createTax(
          new CreationTax(serviceId, citizenId, taxAmount));
      ...
      logging.addLog(
          LogEntry.builder()
              ...
              .message(String.format(
                  ...
                  serviceId, citizenId, e.getMessage()))
              .build());
  ...
  public ResponseEntity<Transaction> payment(final Long paymentId) {
    ...
    try {
      ...
      this.paymentRequestRepository.save(paymentRequest);
      logger.info("new payment request: {}",
                  paymentRequest); ////////////////////
      ...
      logging.addLog(LogEntry.builder()
                         .service("bank")
                         .created(new Date())
                         .message(String.format(
                             "Failed to pay tax for id = %d due %.3900s",
